...
function FilterPost({setpost, postlist}) {
  ...
  return (
    <>
    <section className='flex text-white justify-between w-full md:justify-start lg:w-fit'>
      <span className='hidden bg-inherit md:block mr-1'>Sắp xếp theo:</span> 
      <div className='flex'>
        ...
        <select className='flex text-[var(--primary-text-color)]' defaultValue='desc' onChange={()=>handleSortType()} ref={sortTypeRef}>
          <option value='desc'>Giảm dần</option>
          <option value='asc'>Tăng dần</option>
        </select>
      </div>
    </section>
    <div className='flex text-white w-full md:justify-end items-center lg:w-fit cursor-pointer select-none'> 
        <span className='hidden whitespace-nowrap md:block'>Quick Search:</span>
        <input type="search" onKeyDown={(e)=>handleEnterSearch(e)} ref={searchInputRef} className="mr-1 text-[var(--primary-text-color)] md:mx-1 px-2 py-2 h-8 w-52 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none hover:border-sky-500 focus:border-sky-500 focus:ring-sky-500 block md:rounded-3xl sm:text-sm focus:ring-1" placeholder="Tiêu đề/Tên User" />
        {
          issearch? 
          <button className='whitespace-nowrap bg-[var(--primary-color)] hover:bg-[var(--sub-color)] rounded p-1' onClick={()=>handleSearch()}>Search</button>
          :
          <button className='whitespace-nowrap bg-[var(--primary-color)] hover:bg-[var(--sub-color)] rounded p-1' onClick={()=>handleCancleSearch()}>Hủy Search</button>
        }
    </div>
    </>
  )
